// Runs the frontend smartResource.worker.js in Node.js and returns the final `result` payload as JSON.
...
const workerUrl = new URL('../frontend/src/planning/workers/smartResource.worker.js', import.meta.url);
await import(workerUrl.href);

if (typeof globalThis.self.onmessage !== 'function') {
  throw new Error('smartResource.worker.js did not register self.onmessage');
}
...
// smartResource.worker.js 内部可能通过 setTimeout / async 继续 postMessage（例如补残煤 patched 版本）。
// 这里等待消息队列“稳定”后取最后一次 result，避免只拿到早期占位回包。
