...
def collect_titles(files: list[Path], root: Path) -> list[tuple[str, str]]:
    ...
    for path in files:
        ...
        if title:
            results.append((str(path.relative_to(root)).replace("\\", "/"), title))
...
def collect_urls(files: list[Path], root: Path) -> list[tuple[str, str]]:
    ...
    for path in files:
        ...
        for url in URL_PATTERN.findall(text):
            results.append((str(path.relative_to(root)).replace("\\", "/"), url.rstrip(".,);")))
    unique: list[tuple[str, str]] = []
    seen: set[tuple[str, str]] = set()
    for item in results:
        if item not in seen:
    ...
    for path in files:
        ...
        for pattern in ROUTE_PATTERNS:
            for match in pattern.findall(text):
                route = match.strip()
                if route:
                    results.append((str(path.relative_to(root)).replace("\\", "/"), route))
    unique: list[tuple[str, str]] = []
    seen: set[tuple[str, str]] = set()
    for item in results:
        if item not in seen:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-depth", type=int, default=4, help="扫描最大深度")
    parser.add_argument("--max-files", type=int, default=600, help="最多扫描文本文件数")
    ...
    files = iter_text_files(root, args.max_depth, args.max_files)
